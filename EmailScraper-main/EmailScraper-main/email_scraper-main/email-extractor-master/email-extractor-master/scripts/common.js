const refrech = (onlyBadge = false) => {
    chrome.tabs.query({ currentWindow: true, active: true }, function (tabs) {
        chrome.tabs.sendMessage(tabs[0].id, { tab: tabs[0], data: { onlyBadge: onlyBadge } }, showResults);
    });
}
...
const updateBadge = text => {
    chrome.browserAction.setBadgeBackgroundColor({ color: [17, 125, 208, 1] });
    chrome.browserAction.setBadgeText({ text: text.toString() });
}

const copy = () => {
    chrome.tabs.query({ currentWindow: true, active: true }, function (tabs) {
        chrome.tabs.sendMessage(tabs[0].id, { tab: tabs[0], data: {} }, (res) => {
            showResults(res);
            if (res && res.content) {
                const emailExtractor = new EmailExtractor(res.content);
