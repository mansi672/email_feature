...
let empty = value => {
    return value === undefined || value === null || value === '' || ['Null', 'NULL', '', 'null'].indexOf(value) > -1 ? true : false;
}

let emptyArray = array => {
    return array !== undefined || array instanceof Array || array.length > 0 ? true : false;
}
...
class EmailExtractor {
    ...
    constructor(text) {
        ...
        this.emailDomains = this.emails.map(email => EmailExtractor.getEmailDomain(email));
    ...
    static getEmailDomain(email) {
        return empty(email) ? '' : email.replace(/.*@/, "");
    }
